    ...
    if magic == 0xBC:
      ...
      if format == 0:
          ...
          if ret:
            ...
            cv2.putText(color_img, "Press SPACE to save", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
          # Show the frame
          cv2.imshow("Chessboard Capture", color_img)

          key = cv2.waitKey(1) & 0xFF
          if key == ord(" "):  # Save image when SPACE is pressed
            img_name = os.path.join(save_path, f"calib_{imgcount}.jpg")
            cv2.imwrite(img_name, color_img)
